...
- Enabling `mcp.httpRoute.enabled` or `mcp.ingress.enabled` gives the MCP server its own external route, and the frontend then stops proxying `/mcp` and `/oauth/mcp`. Either route requires `mcp.config.authMode` to be at least `open`, plus its own hostnames (`mcp.httpRoute.parentRefs` and `mcp.httpRoute.hostnames`, or `mcp.ingress.hosts`) — the chart fails rather than publishing a placeholder host.
